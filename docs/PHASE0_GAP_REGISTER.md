...
Scope note (2026-08-17): dry-run and release/hash entries below are historical Phase
0 evidence, not current workflow entry points or current training startup gates.
...
| P0-05 | Reliable patient mapping absent | unavailable and outside current claim scope | Spec 5, Section 8 | authorization patient-state fields and claim audit | no | do not infer; record `not_evaluated`, disallow patient claims, mark gate `NOT APPLICABLE` | preflight/config/documentation negative tests | not applicable |
...
| P0-10 | Training seeds/budget/checkpoint/CI unresolved | TBD | Spec 6 | baseline TOML and protocols | yes | conservative three-seed 20-epoch preregistration; resume requires continuous paired artifacts and complete model/optimizer state | training/eval tests | frozen |
...
| P0-14 | No formal fail-closed entry | missing | Task completion gate | `formal-preflight`, `formal-train`, authorization JSON | yes | training consumes a passing preflight and starts zero steps on any applicable failure | entrypoint tests and passing preflight | resolved; authorization passes |
